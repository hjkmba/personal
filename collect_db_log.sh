...
do
	...
	cp -f $file "$DEST_PATH/$env/$server_name.$log_date.gz" || (errorMsg "$file_name: copy failed"; continue)
	# 4. backup remote files
	mv -f $file "$SOURCE_PATH/backup/$file_name" 2>/dev/null || errorMsg "$file_name: backup failed"
done
